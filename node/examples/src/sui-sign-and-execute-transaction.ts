...
async function main() {
    console.log(`Running sui-sign-and-execute-transaction example...`)
    ...
    try {
        ...
        const res = await new Sui().signAndExecuteTransaction('', txBytes)
        ...
        if (res.cancelled) {
            ...
            console.log(`CompletedAt: ${res.completedAt}, Result: ${res.result}`)
        }
    } catch (err) {
        console.error(`Error during transaction signing and executing, err:${err}`)
        exit(1)
    }
}
